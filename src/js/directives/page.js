 ...
 *perPageOptions    可选择每页显示条数的数组
 *rememberPerPage   使用LocalStorage记住所选择的条目数的名称
 */
 // '第 <input type="text" ng-model="jumpPageNum"  data-ng-keyup="jumpToPage($event)"/> 页 ' +
angular.module('page', []).directive('page',[function(){
    return {
        restrict: 'EA',
        template: '<div class="page-list clearfix">' +
            '<ul class="pagination" ng-show="conf.totalItems > 0">' +
            '<li ng-class="{disabled: conf.currentPage == 0}" data-ng-click="prevPage()"><span>&lt;</span></li>' +
            '<li ng-repeat="item in pageList track by $index" ng-class="{active: (item-1) == conf.currentPage, separate: item == \'...\'}" ' +
            'data-ng-click="changeCurrentPage(item-1)">' +
            '<span>{{ item }}</span>' +
            '</li>' +
            '<li ng-class="{disabled: conf.currentPage == conf.numberOfPages}" data-ng-click="nextPage()"><span>&gt;</span></li>' +
            '</ul>' +
        ...
        link: function(scope, element, attrs){
            ...
            function getPagination(newValue, oldValue) {
                ...
                if(scope.conf.currentPage < 0){
                    scope.conf.currentPage = 0;
                }

                // 如果分页总数>0，并且当前页大于分页总数
                if(scope.conf.numberOfPages > 0 && (scope.conf.currentPage + 1) > scope.conf.numberOfPages){
                    scope.conf.currentPage = scope.conf.numberOfPages - 1;
                }

                // jumpPageNum
                scope.jumpPageNum = scope.conf.currentPage + 1;
            ...
            scope.jumpToPage = function(){
                ...
                if(scope.jumpPageNum !== '' && (scope.jumpPageNum - 1) < scope.conf.numberOfPages){
